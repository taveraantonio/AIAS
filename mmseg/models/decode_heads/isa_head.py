...
class ISALayer(nn.Module):
    ...
    def __init__(self,
        ...
        if in_conv_kernel_size is not None:
            ...
            self.in_conv = None
        self.global_relation = SelfAttentionBlock(channels,
                                                  isa_channels,
                                                  key_query_num_convs=key_query_num_convs,
                                                  conv_cfg=conv_cfg,
                                                  norm_cfg=norm_cfg,
                                                  act_cfg=act_cfg)
        self.local_relation = SelfAttentionBlock(channels,
                                                 isa_channels,
                                                 key_query_num_convs=key_query_num_convs,
                                                 conv_cfg=conv_cfg,
                                                 norm_cfg=norm_cfg,
                                                 act_cfg=act_cfg)
        if out_cat_and_conv:
            self.out_conv = ConvModule(channels * 2,
                                       channels,
                                       kernel_size=1,
                                       conv_cfg=conv_cfg,
                                       norm_cfg=norm_cfg,
...
class ISAHead(BaseDecodeHead):
    ...
    def __init__(self, isa_channels, down_factor=(8, 8), **kwargs):
        ...
        self.in_conv = ConvModule(self.in_channels,
                                  self.channels,
                                  3,
                                  padding=1,
        ...
        self.global_relation = SelfAttentionBlock(self.channels,
                                                  isa_channels,
                                                  conv_cfg=self.conv_cfg,
                                                  norm_cfg=self.norm_cfg,
                                                  act_cfg=self.act_cfg)
        self.local_relation = SelfAttentionBlock(self.channels,
                                                 isa_channels,
                                                 conv_cfg=self.conv_cfg,
                                                 norm_cfg=self.norm_cfg,
                                                 act_cfg=self.act_cfg)
        self.out_conv = ConvModule(self.channels * 2,
                                   self.channels,
                                   1,
                                   conv_cfg=self.conv_cfg,
                                   norm_cfg=self.norm_cfg,

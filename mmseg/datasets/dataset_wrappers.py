...
class ConcatDataset(_ConcatDataset):
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        if len(set([type(ds) for ds in self.datasets])) != 1:
            raise NotImplementedError('All the datasets should have same types when '
            ...
            if mmcv.is_list_of(results, np.ndarray) or mmcv.is_list_of(results, str):
                ...
                gt_seg_maps = chain(*[dataset.get_gt_seg_maps() for dataset in self.datasets])
            else:
                # if the results are `pre_eval` results,
                # we do not need gt_seg_maps to evaluate
                gt_seg_maps = None
            eval_results = self.datasets[0].evaluate(results, gt_seg_maps=gt_seg_maps, logger=logger, **kwargs)
            return eval_results
    ...
    def format_results(self, results, imgfile_prefix, indices=None, **kwargs):
        ...
        for i, indice in enumerate(indices):
            ...
            res = self.datasets[dataset_idx].format_results([results[i]],
